...
def uad_table_etl(file_with_configs : str = 'hpa.cfg') -> pd.DataFrame:
    ...
    uad = fetch_file.submit(folder_name= raw_data_folder + 'FHFA-UAD',url=uad_url)
    hud = fetch_file.submit(folder_name= raw_data_folder + 'HUD-USPS', url=tract_2_zip_url)
    census = fetch_file.submit(folder_name= raw_data_folder + 'census', url=tract20_10_map_url)
    geonames = fetch_file.submit(folder_name= raw_data_folder + 'geonames', url=zip_url)

    zip_uad_df_merged = transform_UAD.submit(wait_for = [uad, hud, census, geonames])

    uad_etl_data_folder = 'data/etl_data/uad_appraisal'
    save_pd_to_parquet(dtframe=zip_uad_df_merged.result(), fldr_name=uad_etl_data_folder, table_name='zip_uad_table', wait_for = zip_uad_df_merged)
    print('ETL of UAD table was successful. Table saved.')
    return zip_uad_df_merged.result()

@flow(name="Redfin, Realtor, Zillow, Zipcode table ETL",
        description = "This sub-flow downloads all data other than UAD, performs etl and saves parquet files.",
        log_prints = True,
        task_runner=DaskTaskRunner(cluster_kwargs={"n_workers": 2,"processes": False})
        )
def house_price_tables(file_with_configs : str = 'hpa.cfg') -> None:
    ...
    realtor = fetch_file.submit(url=realtor_url, folder_name='data/raw_data/realtor_data/')
    realtor_df_slice_agg = transform_realtor.submit(wait_for = [realtor]).result()


    redfin = fetch_file.submit(url=redfin_url, folder_name='data/raw_data/redfin_data/')
    redfin_df_slice_zip_agg = transform_redfin.submit(wait_for = [redfin]).result()

    zillow = fetch_file.submit(url=zillow_url, folder_name='data/raw_data/zillow_data/', fname='Zip_zhvi_uc_sfrcondo_tier_0.33_0.67_sm_sa_month.csv')
    zillow_zhvi_zip_agg, zillow_zhvi_df_slice_zip_unduplicated = transform_zillow.submit(wait_for = [zillow]).result()
